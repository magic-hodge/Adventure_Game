...
#Gets the class user would like to play as, and runs respective class function accordingly.
...
def go_somewhere():
    ...
    if "map" in items:
        ...
        ["armory", "town", "plaza", "howling", "sunken", "tavern", "1", "2", "3", "4", "5"])
        ...
        response = valid_input("Please select a destination.\n",
        ["armory", "town", "plaza", "howling", "cave", "tavern", "1", "2", "3", "4"])
...
def buy_8ball():
    if "8-Ball" not in items:
        response = valid_input("Would you like to purchase the 8-Ball Pistol for $100?\n"
        "Please enter 'yes' or 'no'.\n", ["yes", "no"])
        if "yes" in response:
            if money >= 100:
                money_sub(100)
                items_add("8-Ball")
                print_pause(f"You've purchased the 8-Ball pistol and now have ${money}. Lock 'n load!")
            else:
                print_pause("The armory clerk looks at your change and shakes his head... "
                f"You only have ${money}. You don't have enough for the 8-Ball.")
        elif "no" in response:
...
def town_plaza():
    if "map" not in items:
        print_pause("You wander into the town plaza and see a scruffy citizen.")
        ...
        if "talk" in response or "speak" in response:
            ...
            print_pause("The citizen offers to sell you a treasure map for $25.")
            display_money()
            response = valid_input("Would you like to purchase the treasure map for $25?\n"
            "Please enter 'yes' or 'no'.\n", ["yes", "no"])
            ...
            if 'yes' in response:
                ...
                if money >= 25:
                    ...
                    "\'Wow! You're worse off than me, heh. See ya, scrub!\'")

            elif 'no' in response:
                print_pause("You've decided not to buy the map.\n"
                "The citizen hurries off without even saying goodbye.")
                town_plaza()

        elif "1" in response:
            print_pause("You strike up a conversation...")
            print_pause("The citizen offers to sell you a treasure map for $25.")
            response = valid_input("Would you like to purchase the map for $25?\n"
            "Please enter 'yes' or 'no'.\n", ["yes", "no"])
        ...
        print_pause("You already have the map, so there's nothing else to do in the plaza.")
        go_somewhere()
...
def buy_map():
    if money >= 25:
        money_sub(25)
        items_add("map")
        print_pause(f"You've purchased the treasure map and now have ${money}. Let's get that loot!")
    else:
        print_pause("The townie looks at your change and says... ")
        print_pause(f"You only have ${money}?! Wow. You're worse off than me. See ya, scrub!")

#Player explores howling_cave.
def howling_cave():
    print_pause("You find yourself in Howling Cave. An intimidating noise is coming from within...\n")
...
def fight_werewolf():
    ...
    if "rifle" in items:
        ...
        if "8-Ball" in items:
            print_pause("You take two shots with the 8-Ball pistol, hoping they'll do the trick.")
            print_pause("They connect and slow the werewolf down! But the werewolf continues its assault.")
            if "9-Breaker" in items:
                print_pause("You prepare the 9-Breaker...")
                if "mercenary" in items:
                    print_pause("Fortunately, the mercenary you hired acts as a decoy and tosses you silver bullets.")
                    print_pause("You load the 9-Breaker with the silver bullets and shoot to kill.")
                    print_pause("You've slain the werewolf, and the town has nothing to fear.\n"
                    "Congratulations, you've won!")
                    play_again()
                else:
                    print_pause("But you have no backup and no bullets to load it with...")
            ...
            if "mercenary" in items:
                ...
                print_pause("You and the mercenary have been slain by the werewolf.")
...
def sunken_cave():
    if "9-Breaker" not in items:
        print_pause("You follow the map and come across the location of the sunken cave...")
        print_pause("Fortunately, you know exactly where to look and uncover the legendary pistol, 9-Breaker.")
        items_add("9-Breaker")
        go_somewhere()
    else:
        print_pause("You've been here before... You already have the 9-Breaker "
        "and there's nothing else in Sunken Cave.")
    go_somewhere()

#clues the player in to the citizen with the map.
def gossip():
    print_pause("You sit down at the bar and overhear a conversation...\n")
    print_pause("*Patron 1* Apparently there's a map that reveals the location of the legendary 9-Breaker pistol. . .\n")
    print_pause("*Patron 2* Oh yeah? Well I hear you have to win $25 at rock, paper, scissors to beat the game.\n")
...
def order_drink():
    ...
    response = valid_input("The bartender asks if you'd like to order a drink for $5?\n"
    ...
    if "yes" in response:
        if money >= 5:
            ...
            print_pause("I'm sorry. You don't have enough money to order a drink.")
    elif 'no' in response:
        print_pause("Alright then. Give me a holler if ya change yer mind!")
    tavern()

#Player enters the tavern, and is able to gamble, hire mercenary, buy a drink, gossip, or leave.
def tavern():
    ...
    response = valid_input("Please make a selection.\n",
    ["drink", "mercenary", "gamble", "gossip", "leave", "1", "2", "3", "4", "5"])
...
#Lets player gamble at rock, paper, scissors. Sometimes if player loses, it doesn't type out full message.
def rock_paper_scissors():
    display_money()
    print_pause("Would you like to play rock, paper, scissors and bet $25/game?")
...
#response vs. rock.
            if "rock" in response or '1' in response:
                ...
                if random.choice(opponent) == "scissors":
                    ...
                    print_pause("Your opponent chose 'scissors', so you've won!")
                    money_add(50)
                elif random.choice(opponent) == "rock":
                    print_pause("You chose 'rock'.")
                    print_pause("Your opponent also chose 'rock', so it's a tie.")
                    money_add(25)
...
#response vs. paper.
                ...
                if random.choice(opponent) == "rock":
                    ...
                    print_pause("You chose 'paper'.")
                    print_pause("Your opponent also chose 'paper', so it's a tie.")
                    money_add(25)
                elif random.choice(opponent) == "scissors":
                    print_pause("You chose 'paper'.")
                    print_pause("Your opponent chose 'scissors', so you've lost...")
...
#response vs. scissors.
                ...
                if random.choice(opponent) == "paper":
                    ...
                    print_pause("You chose 'scissors'.")
                    print_pause("Your opponent also chose 'scissors', so it's a tie.")
                    money_add(25)
                elif random.choice(opponent) == "rock":
                    print_pause("You chose 'scissors'.")
                    print_pause("Your opponent chose 'rock', so you've lost...")
...
#Asks to play again. -- Had some weird functionality with the play again function if player wins. . .
def play_again():
